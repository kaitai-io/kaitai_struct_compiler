...
import io.kaitai.struct.{Utils, ClassTypeProvider}
...
class CTranslator(provider: ClassTypeProvider, importList: CppImportList, isInternal: Boolean) extends BaseTranslator(provider) {
  ...
  def doArrayLiteralInternal(t: DataType, value: Seq[expr]): String = {
    ...
    t match {
      ...
      case KaitaiStreamType | OwnedKaitaiStreamType   => s"ks_array_stream_from_data(ks_stream_get_config(stream), $size, $args)"
  ...
  override def strLiteralUnicode(code: Char): String =
    code.toString().getBytes("UTF-8").map(c => "\\%03o".format(c & 0xff)).mkString("")
  ...
  override def genericBinOp(left: Ast.expr, op: Ast.binaryop, right: Ast.expr, extPrec: Int) = {
    op match {
      case Ast.operator.LShift | Ast.operator.RShift =>
        val thisPrec = OPERATOR_PRECEDENCE(op)
        val leftStr = translate(left, thisPrec)
        val rightStr = s"(${translate(right, thisPrec)})"
        val expr = s"$leftStr ${binOp(op.asInstanceOf[Ast.operator])} $rightStr"
        if (thisPrec <= extPrec) s"($expr)" else expr
      case _ =>
        (detectType(left), detectType(right), op) match {
          case (_: IntType, _: IntType, Ast.operator.Mod) => s"ks_mod(stream, ${translate(left)}, ${translate(right)})"
          case (_: IntType, _: IntType, Ast.operator.Div) => s"ks_div(stream, ${translate(left)}, ${translate(right)})"
          case _                                          => super.genericBinOp(left, op, right, extPrec)
        }
    }
  }

  // Instance fields need lazy loading via FIELD(); seq fields can be accessed directly.
  private def instanceFieldTypeOf(classSpec: ClassSpec, attrName: String): Option[DataType] =
    classSpec.instances.collectFirst {
      case (id, spec) if CCompiler.idToStr(id) == attrName => spec.dataTypeComposite
    }

  private def isInstanceFieldOf(classSpec: ClassSpec, attrName: String): Boolean =
    instanceFieldTypeOf(classSpec, attrName).isDefined
  ...
  override def doName(s: String) =
    if (s.startsWith("_")) {
      ...
      if (isInternal && isInstanceFieldOf(provider.nowClass, s)) {
  ...
  override def doEnumByLabel(enumSpec: EnumSpec, label: String): String = {
    val enumClass = enumSpec.name.drop(1).mkString("_")
    s"KSX_${enumClass}_$label".toUpperCase()
  }
  override def doEnumById(enumSpec: EnumSpec, id: String): String = id
  ...
  override def doIfExp(condition: expr, ifTrue: expr, ifFalse: expr): String = {
    val outType = detectType(Ast.expr.IfExp(condition, ifTrue, ifFalse))
    outType match {
      case KaitaiStructType | CalcKaitaiStructType(_) | AnyType => s"(${translate(condition)} ? ((ks_usertype_generic*)${translate(ifTrue)}) : ((ks_usertype_generic*)${translate(ifFalse)}))"
      case _                                                    => s"(${translate(condition)} ? ${translate(ifTrue)} : ${translate(ifFalse)})"
    }
  }
  ...
  override def intToStr(i: expr): String = {
    s"ks_string_from_int(ks_usertype_get_config(&data->kaitai_base), ${translate(i)}, 10)"
  }
  ...
  override def anyField(value: expr, attrName: String): String = {
    val genericPtrExpr = value match {
      case Ast.expr.IfExp(condition, ifTrue, ifFalse) => s"(${translate(condition)} ? ((ks_usertype_generic*)${translate(ifTrue)}) : ((ks_usertype_generic*)${translate(ifFalse)}))"
      case _                                          => s"((ks_usertype_generic*)${translate(value)})"
    }
    attrName match {
      case "_io"     => s"ks_usertype_get_stream($genericPtrExpr)"
      case "_parent" => s"ks_usertype_get_parent($genericPtrExpr)"
      case "_root"   => s"ks_usertype_get_root($genericPtrExpr)"
      case _         =>
        if (!isInternal) {
        ...
        detectType(value) match {
          case t: UserType =>
            val targetClass = t.classSpec.get
            val typeStr = CCompiler.kaitaiType2NativeType(t)
            val fieldType = instanceFieldTypeOf(targetClass, attrName)
            val needsExternalGetter = t.isExternal(provider.nowClass) || (t.isOpaque && targetClass != provider.topClass) // Our own top class is *not* opaque!
            fieldType match {
              case Some(ft) if needsExternalGetter =>
                val retTypeStr = CCompiler.kaitaiType2NativeType(ft) + CCompiler.getPtrSuffix(ft)
                s"""FIELD_EXTERNAL(${translate(value)}, "$attrName", $retTypeStr, $typeStr)"""
              case Some(_) =>
                s"FIELD(${translate(value)}, $typeStr, $attrName)"
              case None =>
                s"((${typeStr}*)${translate(value)})->$attrName"
            }
          case _ => s"ERROR Type"
        }
    }
  ...
  override def doInterpolatedStringLiteral(exprs: Seq[Ast.expr]): String =
    if (exprs.isEmpty) {
      ...
      val cfg = if (isInternal) "ks_usertype_get_config(&data->kaitai_base)" else "config"
      val params = exprs.map { expr =>
        detectType(expr) match {
          case _: IntType => s"ks_string_from_int($cfg, ${translate(expr)}, 10)"
          case _: StrType => translate(expr)
          case otherType  => throw new UnsupportedOperationException(s"unable to convert $otherType to string in format string (only integers and strings are supported)")
        }
      }.mkString(", ")
      s"ks_string_concat(${exprs.size}, $params)"
    }
  override def strConcat(left: Ast.expr, right: Ast.expr, extPrec: Int): String = s"ks_string_concat(2, ${translate(left)}, ${translate(right)})"
